...
savePath = 'G:\Meine Ablage\Studium\Master\Project Border Ownership\FG_RNN\output\feedback\20250922_uniform_coverage_BG_cells\Vase 25';
...
params.B.alpha = 0.36; % exp decay
params.B.beta = 1; % scale
params.B.gamma = 7; % competition
params.B.zeta = 0.7; % shunting inhibition
params.B.lambda = 5; % FB scale

params.G.alpha = 0.25; % exp decay
params.G.beta = 1; % scale
params.G.gamma = 0; % competition (not implemented)
params.G.zeta = 0.9; % shunting inhibition
params.G.mu = 1; % inhibition from opposing B cells

params.G.inhibition_filter = gaussianFilter2D(5, 5, 1, 1);
params.G.RF = makeGRF(params.scale, params.oris+pi/2);

% stimulus = CStimulus(400, 200, 320, 120, 40, 1);
stimulus = vaseStimulus(100, 200, 10, 50);
...
oriensMatrix = mod(round(oriensMatrix/(2*pi)*params.num_ori*2)+params.num_ori/2,params.num_ori*2)+1; % convert orientations to indices

filt = zeros(8, 120, 120);
for ori=1:params.num_ori
    % Combine opposite contrast polarities at each orientation
    E(ori, :, :) = corfresponse.*(oriensMatrix==ori) + corfresponse.*(oriensMatrix==ori+params.num_ori);
...
%% Plots
figure; imagesc(stimulus); axis image; colorbar;
saveas(gcf, fullfile(savePath, 'Stimulus.png'));
figure; imagesc(hsv2rgb(getBOS(B1, B2, params))); axis image;
saveas(gcf, fullfile(savePath, 'BOS.png'));
figure; imagesc(G); axis image; colorbar;
saveas(gcf, fullfile(savePath, 'G.png'));


horizontal_locations = [72,37;];
vertical_locations = [100,43; 85,41];
g_locations = [100,50; 90,50; 70,50];
...
paramText = sprintf([...
    ...
    params.iterations, params.num_ori, params.scale);
